...
def test_user_str(base_user):
    """Test the custome user model string representation"""
...
def test_super_user_is_not_staff(user_factory):
    """Test that a n error is raised when an admin user has is_staff set to false"""
...
def test_create_user_with_no_username(user_factory):
    ...
    assert str(err.value) == "Users must subimit Username"

def test_create_user_with_no_firstname(user_factory):
    """Test that creating a new user with no first_name raises an error """
    with pytest.raises(ValueError) as err: 
        user_factory.create(first_name=None)
    assert str(err.value) == "Users must subimit First_name"

def test_create_user_with_no_last_name(user_factory):
    """Test that creating a new user with no last_name raises an error """
    with pytest.raises(ValueError) as err:
        user_factory.create(last_name=None)
    assert str(err.value) == "Users must subimit Last_name"

def test_create_superuser_with_no_email(user_factory):
    """Test that creating a new superuser with no email address raises an error """
    with pytest.raises(ValueError) as err:
...
def test_create_superuser_with_no_password(user_factory):
    """Test that creating a new superuser with no password raises an error """
    with pytest.raises(ValueError) as err:
...
def test_user_email_incorrect(user_factory):
    """Test that non valid email is provided """
    with pytest.raises(ValueError) as err:
